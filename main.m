...
x_prev = x0;
% Добавим ограничение на максимальное число итераций
max_iter = 100;
iter = 0;
difference = 1;

fprintf('Метод Ньютона:\n');
fprintf('№ итерации |    Корень    |   Разность   |  Точность ε |  T(x_prev)   |  dT(x_prev)\n');
fprintf('-----------|--------------|--------------|-------------|--------------|------------\n');

while difference > epsilon && iter < max_iter
    x_next = x_prev - T(x_prev) / dT(x_prev);
    ...
    fprintf('%10d | %12.6f | %12.6f | %12.6f | %12.6f | %12.6f\n', ...
            iter, x_next, difference, epsilon, T(x_prev), dT(x_prev));

    x_prev = x_next;
end

if iter >= max_iter
    fprintf('\nДостигнуто максимальное число итераций (%d). Цикл прерван.\n', max_iter);
else
